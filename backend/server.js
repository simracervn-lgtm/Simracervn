...
app.post('/api/orders', async (req,res)=>{
  try{
    ...
    if(!name || !phone || !items?.length) return res.status(400).json({error:'Thiếu thông tin'});
    const {data, error} = await supabase.from('orders').insert([{customer_name: name, phone, address, note, items, total}]).select().single();
